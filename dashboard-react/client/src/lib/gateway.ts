...
export type CartelaSelection = { ok: boolean; play_wallet?: number; selected_cartelas?: number[]; reserved_cartelas?: number[]; pending_revision?: number };
...
export const playerApi = {
  ...
  selectCartela: (roundId: string, userId: string | number, cartelaNumber: number) => gatewayFetch<CartelaSelection>(`/api/rounds/${encodeURIComponent(roundId)}/select`, { method: "POST", body: JSON.stringify({ user_id: Number(userId), cartela_number: cartelaNumber }) }),
  unselectCartela: (roundId: string, userId: string | number, cartelaNumber: number) => gatewayFetch<CartelaSelection>(`/api/rounds/${encodeURIComponent(roundId)}/unselect`, { method: "POST", body: JSON.stringify({ user_id: Number(userId), cartela_number: cartelaNumber }) }),
