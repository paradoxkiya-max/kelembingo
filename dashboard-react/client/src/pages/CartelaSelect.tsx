...
export default function CartelaSelect() {
  ...
  useEffect(() => {
    if (seconds <= 0) {
      if (selected.length) void confirmSelection();
      return;
    }
  ...
  async function toggleCard(number: number) {
    if (busy || taken.has(number) || !player?.user_id) return;
  ...
  async function confirmSelection() {
    ...
    try {
      ...
      await playerApi.joinRound(activeRound.id, player.user_id, selected, player.first_name || "Player");
      navigate(`/game?round=${encodeURIComponent(activeRound.id)}`);
    } catch (e) {
  ...
  return <div className="flex min-h-[calc(100vh-56px)] flex-col bg-[linear-gradient(180deg,#0d0f22_0%,#151833_40%,#0d0f22_100%)]">
    ...
    <div className="flex items-center justify-between gap-1 border-b border-white/5 bg-[#111326]/60 px-4 py-3 text-[11px] font-semibold text-gray-300"><div className="flex gap-2"><Summary label="PLAY WALLET" value={`${wallet.toLocaleString()} ETB`} tone="text-[#34D399]" /><Summary label="STAKE" value={`${stake} ETB`} tone="text-[#FF8C00]" /><Summary label="EST. DERASH" value={`${poolEstimate} ETB`} tone="text-[#8B5CF6]" /></div><div className="relative flex min-w-[55px] items-center justify-center overflow-hidden rounded-lg border border-emerald-500/30 bg-emerald-600/20 px-3.5 py-1.5 text-emerald-400"><div className="absolute inset-y-0 left-0 bg-gradient-to-r from-[#10B981] to-[#34D399] opacity-30 transition-[width] duration-300" style={{ width: `${Math.min(100, Math.max(0, (seconds / SELECTION_SECONDS) * 100))}%` }} /><span className="relative z-10 text-xs font-bold">{seconds}s</span></div></div>
    <div className="card-select-grid-enhanced flex-1 overflow-y-auto px-2 py-2" aria-label="Available cartelas">{loading ? <div className="flex items-center justify-center py-16 text-sm text-white/35"><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Loading cartelas</div> : error ? <div className="px-4 py-12 text-center text-xs text-red-300">{error}</div> : <div className="grid grid-cols-8 content-start gap-1.5 max-[360px]:grid-cols-7 max-[360px]:gap-1">{visibleCartelas.map((card) => { const isSelected = selected.includes(card.number); const isTaken = taken.has(card.number) || Boolean(card.taken) || card.status === "taken"; return <button key={card.number} disabled={isTaken} onClick={() => void toggleCard(card.number)} aria-label={`Cartela ${card.number}${isTaken ? ", taken" : isSelected ? ", selected" : ""}`} className={`relative aspect-square rounded-lg border text-[13px] font-extrabold transition-transform active:scale-[0.92] ${isTaken ? "pointer-events-none border-[#FF8C00]/10 bg-[#FF8C00]/10 text-[#FF8C00]/30 opacity-50" : isSelected ? "z-[1] scale-[1.04] border-emerald-400/60 bg-gradient-to-br from-[#10B981] to-[#059669] text-white shadow-[0_0_16px_rgba(16,185,129,0.45)]" : "border-white/10 bg-gradient-to-br from-[#1E2340] to-[#151833] text-white shadow-[0_2px_8px_rgba(0,0,0,0.3)]"}`}>{isSelected ? <Check className="mx-auto h-4 w-4" /> : card.number}</button>; })}</div>}</div>
    {selected.length > 0 && <div className="border-t border-white/5 bg-[#0e1026] px-3 py-2"><div className="flex justify-center gap-2">{selected.map((number) => { const card = cartelas.find((item) => item.number === number); return <MiniPreview key={number} card={card} />; })}</div></div>}
    ...
    <div className="border-t border-white/5 bg-[#0a0f1d] pb-4"><div className={`items-center justify-between px-4 py-2 text-xs ${selected.length ? "flex" : "hidden"}`}><span className="font-semibold text-gray-400">Selected: <span className="font-bold text-emerald-400">{selected.length}/{MAX_SELECTIONS}</span> cards</span><span className="font-semibold text-gray-400">Total Cost: <span className="font-bold text-orange-400">{selected.length * stake} ETB</span></span></div><div className="px-4 py-2"><button onClick={() => navigate("/")} disabled={busy} className="w-full rounded-xl bg-white/10 py-3 text-sm font-bold text-white transition-colors hover:bg-white/20 disabled:opacity-50">Cancel</button></div></div>
  </div>;
}
