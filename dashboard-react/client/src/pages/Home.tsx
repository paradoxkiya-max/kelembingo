...
export default function Home() {
  ...
  const { player, stats, loading, telegramState, authError } = usePlayer();
  const wallet = walletValue(player?.play_wallet);

  function play(stake: number) {
    if (!player) {
      const reason = telegramState === "browser" ? "Open KelemBingo inside Telegram to authenticate and play with your real wallet." : telegramState === "telegram-no-init-data" ? "Telegram did not provide a valid WebApp session. Reopen KelemBingo from the Telegram bot." : telegramState === "authenticating" ? "Telegram authentication is still loading. Please try again in a moment." : authError || "Telegram authentication failed. Reopen KelemBingo from the Telegram bot.";
      toast.error(reason);
      return;
    }
  ...
  return <div className="min-h-[calc(100vh-56px)] bg-[linear-gradient(180deg,#0D1117_0%,#0A1628_40%,#111827_100%)] pb-4">
    <div className="flex items-center justify-between px-4 py-4"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-[#FF8C00] to-[#FF6B00] text-xl font-black text-white">B</div><div><h1 className="text-xl font-bold text-white">Kelem Bingo</h1><p className="text-xs text-white/50">Play &amp; Win Big!</p></div></div><button onClick={() => toast.info("Choose a stake, select up to two cartelas, then follow the live calls to complete a row, column, or diagonal.")} className="flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-white/[0.06] text-white/70 hover:text-white" aria-label="Show game rules"><Info className="h-5 w-5" /></button></div>
    <section className="mb-6 px-4"><div className="glass rounded-2xl border border-white/10 bg-[#1A1A2E]/75 p-5 text-center"><div className="mb-2 text-3xl">🎯</div><h2 className="mb-1 text-lg font-bold text-white">Welcome to <span className="text-[#FF8C00]">Kelem Bingo</span></h2><p className="text-sm text-white/60">Pick your stake and start playing!</p><div className="mt-2 text-xs font-medium text-[#14B8A6]">{player ? `Hello, ${player.first_name || "player"}!` : telegramState === "browser" ? "Open inside Telegram to play" : telegramState === "authenticating" ? "Connecting Telegram account…" : telegramState === "auth-failed" ? "Telegram authentication failed" : "Telegram session not ready"}</div></div></section>
    <section className="mb-6 px-4"><div className="grid grid-cols-2 gap-3"><button onClick={() => play(10)} disabled={loading} className="rounded-2xl bg-gradient-to-br from-[#10B981] to-[#059669] p-4 text-center shadow-[0_8px_25px_rgba(16,185,129,0.3)] transition-transform active:scale-[0.97] disabled:cursor-wait disabled:opacity-55"><div className="mb-1 text-2xl">🎮</div><div className="text-lg font-black text-white">10 ETB</div><div className="mt-1 text-[10px] text-white/80">Standard</div></button><button onClick={() => play(20)} disabled={loading} className="rounded-2xl bg-gradient-to-br from-[#8B5CF6] to-[#6D28D9] p-4 text-center shadow-[0_8px_25px_rgba(139,92,246,0.3)] transition-transform active:scale-[0.97] disabled:cursor-wait disabled:opacity-55"><div className="mb-1 text-2xl">🎯</div><div className="text-lg font-black text-white">20 ETB</div><div className="mt-1 text-[10px] text-white/80">High Stakes</div></button></div><p className="mt-2 text-center text-[10px] text-white/40">Max 2 cartelas · Derash = (Cartelas × Stake × 0.80) / Winners</p></section>
    {telegramState !== "authenticated" && !loading && <div className="mx-4 mb-6 rounded-xl border border-[#F59E0B]/25 bg-[#F59E0B]/10 px-3 py-2 text-center text-[11px] text-[#FCD34D]">{telegramState === "browser" ? "Preview mode: open inside Telegram to authenticate and play with your real wallet." : telegramState === "telegram-no-init-data" ? "Telegram WebApp session was not provided. Reopen this game from Telegram." : telegramState === "auth-failed" ? `Telegram authentication failed${authError ? `: ${authError}` : "."}` : "Connecting to Telegram…"}</div>}
