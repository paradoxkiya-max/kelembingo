...
export default function GameBoard() {
  ...
  // WinnerAnnouncement renders the authoritative server payout as PRIZE PER WINNER.
  ...
  const playerCartelaNumbers = useMemo(() => playerCartelaKey.split(",").map(Number).filter((number) => Number.isInteger(number) && number > 0), [playerCartelaKey]);
  ...
  useEffect(() => {
    ...
    setCardError("");
    // The backend generator is deterministic by cartela number, so the player
    // gets an immediate exact card while the server copy hydrates in background.
    setCartelas(ids.map((number) => fallbackCartela(number)));
    Promise.all(ids.map(async (number) => ({ number, card: (await playerApi.cartela(number)).cartela })))
      .then((items) => {
        if (!active) return;
        const validItems = items.filter(({ number, card }) => isValidCartela(card, number));
        setCartelas(validItems.map(({ card }) => card as Cartela).concat(items.filter(({ number, card }) => !isValidCartela(card, number)).map(({ number }) => fallbackCartela(number))));
        if (validItems.length < ids.length) setCardError("Showing the verified local cartela while the server card is unavailable.");
      })
      .catch(() => { if (active) setCardError("Showing the verified local cartela while the server card is unavailable."); })
  ...
  const isSpectator = !hasPlayerEntry || playerCartelaNumbers.length === 0;
...
function CartelaLoadingCard({ number }: { number: number }) { return <div className="overflow-hidden rounded-xl border border-orange-400/30"><div className="bg-gradient-to-br from-[#FF8C00] to-[#FF6B00] py-1 text-center text-[10px] font-black tracking-wider text-white">CARTELA NO: {number}</div><div className="flex min-h-[118px] items-center justify-center gap-1.5 bg-[#1A1A2E] text-[9px] text-white/40"><Loader2 className="h-3 w-3 animate-spin text-[#FF8C00]" /> Syncing card</div></div>; }

function CartelaCard({ card, marked, called, onMark }: { card: Cartela; marked: Set<number>; called: Set<number>; onMark: (number: number) => void }) { const values = cardValues(card, card.number); return <div className="cartela-container overflow-hidden rounded-xl border border-orange-400/30"><div className="cartela-header bg-gradient-to-br from-[#FF8C00] to-[#FF6B00] py-1 text-center text-[10px] font-black tracking-wider text-white">CARTELA NO: {card.number}</div><div className="grid grid-cols-5 gap-px" style={{ background: "rgba(26,26,46,0.5)" }}>{letters.map((letter, index) => <div key={letter} className="py-0.5 text-center text-[8px] font-black text-white" style={{ background: colors[index] }}>{letter}</div>)}{values.map((number, index) => { const isMarked = marked.has(number) || index === 12; return <button key={`${card.number}-${index}`} onClick={() => onMark(number)} className={`cartela-cell aspect-square text-[9px] font-black transition-transform active:scale-90 ${isMarked ? "marked bg-gradient-to-br from-[#10B981] to-[#059669] text-white shadow-[0_0_8px_rgba(16,185,129,0.3)]" : "bg-[rgba(30,35,64,0.8)] text-white/75"}`} disabled={!called.has(number) && index !== 12}>{index === 12 ? "★" : number}</button>; })}</div></div>; }
function roundDerashPool(round: Round) { const cartelas = Math.max(0, Number(round.player_count) || 0); const stake = Math.max(0, Number(round.stake) || 0); if (round.status === "playing") return Math.round(cartelas * stake * 0.80 * 100) / 100; const authoritative = Number(round.derash); if (Number.isFinite(authoritative) && authoritative >= 0) return authoritative; return Math.round(cartelas * stake * 0.80 * 100) / 100; }
function flattenCartela(card?: Cartela) { return cardValues(card, card?.number); }
