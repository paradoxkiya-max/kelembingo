...
export default function GameBoard() {
  ...
  async function claim(number: number, attemptKey?: string) {
    ...
    try {
      ...
      setResult(response.winner ? { winner: true, payout: response.prize_per_winner ?? round?.prize_per_winner ?? 0 } : { winner: false, message: "The server did not validate this cartela." });
    } catch (error) {
      if (attemptKey) claimAttempts.current.delete(attemptKey);
      setResult({ winner: false, message: error instanceof Error ? error.message : "Claim failed" });
  ...
  const roundStatusLabel = round.status === "completed" ? "Round complete" : round.status === "selecting" ? `Selection ${countdown > 0 ? `${countdown}s` : "starting"}` : isSpectator ? "Spectating live" : "Live game";
  const displayTimer = round.status === "selecting" ? (countdown > 0 ? `${countdown}s` : "Starting…") : timer > 0 ? `${timer}s` : "Syncing…";
  ...
  return <div className="flex min-h-[calc(100vh-56px)] flex-col bg-[linear-gradient(180deg,#0D1117_0%,#0A0F18_50%,#111827_100%)] px-2 py-1">
    ...
    {round.status === "selecting" && <div className="mt-1 rounded-xl border border-purple-400/30 bg-purple-500/20 px-4 py-2 text-center text-xs font-black text-purple-200">Game starting soon · selection {countdown > 0 ? `${countdown}s` : "syncing"}</div>}
    {round.status === "completed" && <div className="mt-1 rounded-xl border border-emerald-400/25 bg-emerald-500/10 px-4 py-2 text-center text-xs font-black text-emerald-200">This round is complete. The next round is open from the Game tab.</div>}
